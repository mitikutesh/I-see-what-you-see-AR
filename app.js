...
const statusEl=$('#status'),resultCard=$('#resultCard'),toast=$('#toast'),askPanel=$('#askPanel');
...
function openAskPanel(){askPanel.classList.add('open');askPanel.setAttribute('aria-hidden','false')}
...
async function analyze(image){
  ...
  try{
    ...
    console.error(e);
    renderResult({title:'Image captured',description:'The camera captured your image, but the AI endpoint did not return a result.',icon:'✦',confidence:'ERROR',facts:['Image captured','Check Vercel logs','Check OPENAI_API_KEY']});
...
function analyzeCurrentView(){const img=captureVideo();if(img)analyze(img)}
function renderResult(data){
  lastResult=data;
  $('#resultTitle').textContent=data.title||'I see something';
...
async function askQuestion(){
  ...
  catch(e){console.error(e);answer.textContent='The AI question service failed. Check the Vercel function logs and OPENAI_API_KEY.'}
}

$('#startBtn').addEventListener('click',startCamera);
$('#closeBtn').addEventListener('click',stopCamera);
$('#switchBtn').addEventListener('click',()=>{facing=facing==='environment'?'user':'environment';startCamera()});
$('#captureBtn').addEventListener('click',analyzeCurrentView);
